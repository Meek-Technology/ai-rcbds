...
- **Heterogeneous Multi-Load Engineering Engine**: Full support for single-span and multi-span continuous systems carrying combinations of:
  ...
  - Beam Self-Weight Override (`ignore beam self weight` / $n_2 = 0.0\text{ kN/m}$)
  - Factored wall line loads (density $\times$ thickness $\times$ height)
...
### 7. Overhang Beam (Single Point Load)
`Overhang beam span 6m overhang 2m, point load 20kN at 8m`

### 8. Overhang Beam (UDL + Free End Point Load)
`Overhang beam span 6m overhang 2m, UDL 15kN/m on span and point load 10kN at free end`

### 9. Overhang Beam (Multiple Point Loads)
`Overhang beam 5m span, 1.5m overhang, point loads 20kN at 3m and 15kN at 6.5m`

### 10. Continuous Beam (Single UDL - Ignore Self-Weight)
`Analyze the continuous beam ABC. Support A is fixed, while B and C are roller supports. Span AB is 3 m and span BC is 4 m. UDL 15kN/m on span AB, ignore beam self weight`

### 11. Continuous Beam (3 Spans with Heterogeneous Loading & Fixed Ends)
...
### 12. Continuous Beam (2 Spans with Midpoint Load & Range UDL)
...
### 13. Cantilever Beam (Single Point Load)
`Design a cantilever beam with span 4m, point load of 25kN at 4m`

### 14. Cantilever Beam (Combined Loads)
